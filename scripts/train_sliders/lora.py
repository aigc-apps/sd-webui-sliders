...
class LoRAModule(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("alpha", torch.tensor(alpha))  # regard as the constant
        ...
        for name, module in root_module.named_modules():
            if train_method == "noxattn" or train_method == "noxattn-hspace" or train_method == "noxattn-hspace-last":  # Cross Attention と Time Embed 以外学習
                if "attn2" in name or "time_embed" in name:
                    continue
            elif train_method == "innoxattn":  # Except for Cross Attention
                if "attn2" in name:
                    continue
            elif train_method == "selfattn":  # Self Attention only
                if "attn1" not in name:
                    continue
            elif train_method == "xattn" or train_method == "xattn-strict":  # Cross Attention only
                if "attn2" not in name:
                    continue
            elif train_method == "full":  # All
                pass
            else:
