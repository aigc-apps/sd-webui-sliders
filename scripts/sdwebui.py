...
from scripts.sliders_config import models_path
from scripts.sliders_utils import (
    check_sliders_valid,
    LoraCtlNetwork,
    apply,
    ctl_lora_flag,
...
if ctl_lora_flag:
    ...
    class LoraCtlScript(scripts.Script):
        ...
        def ui(self, is_img2img):
            ...
            with gr.Group():
                with gr.Accordion("Additional Slider Networks", open=False):
                    with gr.Row():
                    ...
                    for i in range(MAX_MODEL_COUNT):
                        with FormRow(variant="compact"):
                            ...
                            for _sliders_id in _sliders_ids:
                                if check_sliders_valid(_sliders_id):
                                    ids.append(os.path.splitext(_sliders_id)[0])
                            ids = sorted(ids)
                            model = gr.Dropdown(list(ids), label=f"Model {i+1}", value="None")
                    ...
                    def refresh_all_models(*dropdowns):
                        ...
                        for dd in dropdowns:
                            ...
                            if dd in ids:
                                ...
                                selected = "None"
                            update = gr.Dropdown.update(value=selected, choices=list(ids))
        ...
        def before_process_batch(self, p: StableDiffusionProcessing, *args, **kwargs):
            if args[0]:
                for i in range(1, len(args) - 1, 2): 
                    model, weight = args[i:i+2]
                    if model != "None":
                        for p_index in range(len(p.prompts)):
                            p.prompts[p_index] += f'<lora:{model}:0@0, 0@0.5, {weight}@0.5, {weight}@1>'

        def process(self, p: StableDiffusionProcessing, *args, **kwargs):
            if "LoraCtlNetwork" not in str(type(extra_networks.extra_network_registry["lora"])):
